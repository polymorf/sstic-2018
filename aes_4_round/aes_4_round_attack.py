...
def integral(ciphertexts):
    ...
    for i in range(len(candidates)):
        if len(candidates[i]) == 0:
            #print "no candidates for %d" % i
            candidates[i] = range(0x100)
    ...
    for roundKey in product(*candidates):
        ...
        if 'D1D3C0DE41414141'.decode("hex")[::-1] in plain:
            #print 'solved', masterKey
            return "".join(map(chr,masterKey))
    return None
...
def attack(ciphertexts):
    ...
    return integral(ciphertexts)


def get_ciphertexts(data):
    ciphertexts = {}
    while len(data):
        ...
        iv=bloc[:0x10].encode("hex")
        ciphertext = bloc[0x10:0x20].encode("hex")
        ciphertexts[iv] = ciphertext
    return ciphertexts

def attack_ciphertext(ciphertexts):
    ciphertexts_ordered = []
    for i,iv in enumerate(sorted(ciphertexts.keys())):
        ciphertexts_ordered.append(ciphertexts[iv])
        if i >= 0xFF:
            break
    return attack(ciphertexts_ordered)

if __name__=="__main__":
    data = open("../1/flux/192.168.231.123.49734-192.168.023.213.31337","rb").read()[0x200:]
    client_ciphertexts = get_ciphertexts(data)
    key = attack_ciphertext(client_ciphertexts)
    if key != None:
        ok("[client -> server] key is : %s" % (key.encode("hex")))
    else:
        error("attack failed :(")

    data = open("../1/flux/192.168.023.213.31337-192.168.231.123.49734","rb").read()[0x200:]
    server_ciphertexts = get_ciphertexts(data)
    key = attack_ciphertext(server_ciphertexts)
    if key != None:
        ok("[server -> client] key is : %s" % (key.encode("hex")))
    else:
        error("attack failed :(")
