...
uint8_t * xor(uint8_t *dst, uint8_t *xor_with, uint32_t len) {
	int i;
	for(i=0;i<len;i++) {
		dst[i] ^= xor_with[i];
	}
...
int main(int argc, char **argv) {
	...
	hexdump(ctx,CTX_SIZE,"context",0);

	memcpy(iv,msg,16);

	int fd2 = open(argv[2],O_WRONLY | O_CREAT, 0x1FF);
	if (fd2 <= 0) {
		err("can't open dump\n");
		return 0;
	}
